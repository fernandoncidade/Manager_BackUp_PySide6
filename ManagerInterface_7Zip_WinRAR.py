...
from CompressionMotors_7Zip_WinRAR import (buscar_sevenzip_executavel, buscar_winrar_executavel, CompressaoRAR,
...
class GerenciadorInterface(QThread):
    def __init__(self, main_window):
        ...
        self.teste_integridade_thread = TestarIntegridade(self.winrar_executable, self.sevenzip_executable, self.compressed_files)
    ...
    def browse_folder(self, main_window):
        ...
        # Definindo a folha de estilo CSS para a caixa de diálogo
    ...
    def testar_integridade(self):
        ...
        if self.winrar_executable and compressed_files:
            self.teste_integridade_thread = TestarIntegridade(self.winrar_executable, self.sevenzip_executable, selected_files)
            self.teste_integridade_thread.finished.connect(self.on_teste_integridade_finished)
            self.teste_integridade_thread.start()

        elif self.sevenzip_executable and compressed_files:
            self.teste_integridade_thread = TestarIntegridade(self.sevenzip_executable, self.winrar_executable, selected_files)
            self.teste_integridade_thread.finished.connect(self.on_teste_integridade_finished)
            self.teste_integridade_thread.start()
